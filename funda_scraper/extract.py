...
class DataExtractor(object):
    ...
    def extract_data(
        self, search_request: SearchRequest, run_id: str, clean_data: bool
    ) -> pd.DataFrame:
        ...
        for page in tqdm(detail_pages, desc="Processing detail pages.."):
            try:
                ...
                logger.error(
                    f"An error occurred while processing house: {e}; skipping this house"
                )
        ...
        logger.info(
            f"There were {len(houses_with_processing_errors)} houses that could not be processed"
        )
        ...
        df = pd.DataFrame(
            [
                {
                    **{k: v for k, v in vars(house).items() if k != "photos"},
                    "photos": house.photos_string,
                }
                for house in houses
            ]
        )
    ...
    def extract_data_from_detail_page(
        self, page: str, search_request: SearchRequest
    ) -> Property:
        ...
        house.year_of_construction = self.get_value_from_css(
            soup, self.selectors.year_of_construction
        )
        ...
        house.building_type = self.get_value_from_css(
            soup, self.selectors.building_type
        )
        house.number_of_rooms = self.get_value_from_css(
            soup, self.selectors.num_of_rooms
        )
        house.number_of_bathrooms = self.get_value_from_css(
            soup, self.selectors.num_of_bathrooms
        )
        ...
        house.neighborhood_name = self.get_value_from_css(
            soup, self.selectors.neighborhood_name
        )
        ...
        house.last_ask_price = self.get_value_from_css(
            soup, self.selectors.last_ask_price
        )
        house.last_ask_price_m2 = self.get_value_from_css(
            soup, self.selectors.last_ask_price_m2
        ).split("\r")[0]
